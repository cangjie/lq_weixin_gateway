...
<script runat="server">
    ...
    public XmlDocument GetReplyMessage(string MsgId)
    {
        ...
        //File.AppendAllText(Server.MapPath("log/login.txt"), DateTime.Now.ToString() + "  " + opType.ToString().Trim() + "\r\n");
        //switch
        switch (opType)
        { 
            case 1:
                ...
                string subUrl = "";
                //File.AppendAllText(Server.MapPath("log/json.txt"), evntKey + "\r\n");
                bool isWenDaMenuPostBack = true;
                ...
                else
                {
                    ...
                    string msgContent = "<item>"
                        ...
                        + "<Description><![CDATA[玩超级大脑，赢取卢勤老师最新力作！只要你的分数达到180分以上，就有机会赢得一本《男孩梦》&《女孩梦》，还在等什么，快来参与吧！]]></Description>"
                        + "<PicUrl><![CDATA[http://gameapi.luqinwenda.com/image/start_all_2.jpg]]></PicUrl>"
                        + "<Url><![CDATA[" + subUrl+"?openid=" + openId.Trim() + "]]></Url>"
                        + "</item>";
                    cmdIns.Parameters.Add("@content", msgContent);
                }
                break;

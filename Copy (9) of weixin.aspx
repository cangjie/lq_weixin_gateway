    ...
    public XmlDocument GetReplyMessage(string MsgId)
    {
        ...
        if (type.Trim().ToLower().Equals("text"))
        {
            ...
            if (text.Trim().Equals("二维码"))
            {
                ...
                File.AppendAllText(Server.MapPath("log/qrerr.log"), DateTime.Now.ToString() + "\t" + "token=" + token + "\r\n");
                
                long scene = long.Parse(Util.GetInviteCode(to.Trim()));
                File.AppendAllText(Server.MapPath("log/qrerr.log"), DateTime.Now.ToString() + "\t" + "scene=" + scene.ToString() + "\r\n");
                string ticket = Util.GetQrCodeTicketTemp(token, scene);
                File.AppendAllText(Server.MapPath("log/qrerr.log"), DateTime.Now.ToString() + "\t" + "ticket=" + ticket.ToString() + "\r\n");
                byte[] qrCodeByteArr = Util.GetQrCodeByTicket(ticket);
                string filePathName = Server.MapPath("images/qrcode/" + scene.ToString() + ".jpg");

                Stream s = File.OpenWrite(filePathName);
                s.Write(qrCodeByteArr, 0, qrCodeByteArr.Length);
                ...
                cmdIns.Parameters.Add("@msgtype", "image");
                cmdIns.Parameters.Add("@content", mediaId);

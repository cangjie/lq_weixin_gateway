...
public class NewYearBox
{
    ...
    */
    ...
    {
        DataTable dt = DBHelper.GetDataTable(" select * from new_year_box_master where [id] = " + id.ToString().Trim(), Util.ConnectionStringGame);
        if (dt.Rows.Count == 1)
            _field = dt.Rows[0];
        else
	...
	{
        DataTable dt = DBHelper.GetDataTable(" select * from new_year_box_master where open_id = '" + openId.Trim() + "' and act_id = " + actId.ToString().Trim(), Util.ConnectionStringGame);
        if (dt.Rows.Count == 0)
        {
            DataTable dtMax = DBHelper.GetDataTable(" select max([id]) from new_year_box_master ", Util.ConnectionStringGame.Trim());
            ...
            int i = DBHelper.InsertData("new_year_box_master", insertParameter, Util.ConnectionStringGame.Trim());
            if (i == 1)
            {
                dt = DBHelper.GetDataTable(" select * from new_year_box_master where open_id = '" + openId.Trim() + "' and act_id =  " + actId.ToString().Trim(), Util.ConnectionStringGame);
                _field = dt.Rows[0];
            }
        }
        else
    ...
    public bool OpenABox(int id)
    {
        ...
        if (CurrentSupportNumber >= neededSupportNumber)
        {
            ...
            int i = DBHelper.InsertData("new_year_box_detail", insertParameters, Util.ConnectionStringGame);
            if (i == 1)
            {
    ...
    {
        DataTable dt = DBHelper.GetDataTable(" select * from new_year_box_detail where master_id = " + ID.ToString(), Util.ConnectionStringGame);
    ...
    public KeyValuePair<int, string>[] GetOpenedBoxWithGift()
    {
        DataTable dt = DBHelper.GetDataTable(" select * from new_year_box_detail where master_id = " 
            + ID.ToString() + "  order by  create_date_time desc ", Util.ConnectionStringGame);
    ...
    public bool Support(string openId, string source)
    {
        ...
        DataTable dt = DBHelper.GetDataTable(" select * from new_year_box_support_list where master_id = " + ID.ToString()
            + " and open_id = '" + openId.Trim() + "'  ", Util.ConnectionStringGame);
        if (dt.Rows.Count == 0)
        {
            string[,] insertParameters = { { "master_id", "int", ID.ToString() }, 
            ...
            int i = DBHelper.InsertData("new_year_box_support_list", insertParameters, Util.ConnectionStringGame);
            if (i == 1)
            {
    ...
    {
        DataTable dt = DBHelper.GetDataTable(" select * from new_year_box_support_list  where master_id = " + ID.ToString(), Util.ConnectionStringGame);
    ...
    public int CurrentSupportNumber
    {
        ...
        set
        {
            ...
            DBHelper.UpdateData("new_year_box_master", updateParameter, keyParameter, Util.ConnectionStringGame);
        }
    }
    ...
    */
    ...
    {
        DataTable dt = DBHelper.GetDataTable(" select * from new_year_box_detail where award_name = '" + awardName.Replace("'", "").Trim() + "' ", Util.ConnectionStringGame);
